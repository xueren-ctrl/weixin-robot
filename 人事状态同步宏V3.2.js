...
function _V32_FlushTables(ctx) {
  ...
  for (var i = 0; i < ctx.tables.length; i++) {
    ...
    var n = info.grid.length;
    // 写入前先记录工作表原有数据末尾（用于写后清理多余尾行）
    var oldLast = info.wasLast;
    if (oldLast === undefined) {
      try { oldLast = _V32_LastRowIn(info.ws, t.nameCol); } catch (e0) { oldLast = 0; }
      var l2 = 0, l3 = 0;
    ...
    // ★ 关键修复：网格缩小（删过行）后，必须清掉尾部残留行，
    //   否则被删除的记录仍留在工作表原有末尾 → 看起来“删除无效”
    var newLast = t.start + n - 1;
    if (oldLast > newLast) {
      try { info.ws.Range((newLast + 1) + ":" + oldLast).ClearContents(); } catch (e3) {}
    }
